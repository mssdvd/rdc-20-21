...
int main()
{
	...
	while(1){
		...
		if (strcmp(method, "POST") == 0 &&
			...
			// Parsing command
			i = 0;
			for (; command[i] != '='; i++); cmd = command + i + 1; 
			for(i++; command[i] != '&'; i++); command[i] = 0;
			for(i++; command[i] != '='; i++); param1 = command + i + 1; 
			for(i++; command[i] != '&'; i++); command[i] = 0;
			for(i++; command[i] != '='; i++); param2 = command + i + 1;
			for(i++; command[i] != 0; i++); command[i] = 0;
